/**
 * WB Parser Shared Utilities
 * Extracted from background.js (1000fps WB Parser)
...
// Multi-format stock quantity extraction from WB API responses
...
// Extract delivery time ranges from WB API responses
...
// Combined extraction into a unified format (WB Parser format)
...
export function extractProductData(article: string, data: unknown): WbProductData | null {
  ...
  return {
    ...
    brand: (product.brand as string) || (product.brandName as string) ||
      (product.selling as Record<string, string>)?.brand_name || '',
    price,
    originalPrice,
...
// ===================== NORMALIZE PARSER RESULT =====================
// Normalize Chrome Extension parser result to our internal format

export interface NormalizedParserResult {
  source: string;
  price: number | null;
...
export function normalizeParserResult(article: string, source: string, rawData: unknown): NormalizedParserResult | null {
  const productData = extractProductData(article, rawData);
  ...
  return {
    ...
    deliveryMin: productData.deliveryMin,
    deliveryMax: productData.deliveryMax,
