...
function updateStats(store) {
  ...
  if (store.lastChecked) {
    document.getElementById("last-check").textContent =
      "Обновлено: " + formatTime(store.lastChecked);
  }
}
  ...
  chrome.runtime.sendMessage(
    ...
    function (res) {
      ...
      if (!res.ok) {
        ...
        setHint(
          msg.includes("вкладк")
            ? "Откройте вкладку wildberries.ru и попробуйте снова"
            : "Ошибка: " + msg,
          "error",
        );
        return;
...
function buildCard(p) {
  ...
  card.className =
    "card" + (diff > 0 ? " price-up" : diff < 0 ? " price-down" : "");
  ...
  if (diff !== 0) {
    ...
    changeBadge =
      '<span class="change-badge ' + cls + '">' + sign + diff + " ₽</span>";
  ...
  if (totalDiff !== 0 && p.history && p.history.length > 2) {
    ...
    totalBadge =
      ...
      cls +
      '" style="font-size:10px;opacity:.7" title="Общее изменение">' +
      sign +
      totalDiff +
      " ₽ всего</span>";
  }
  ...
  const feedbacks = p.feedbacks
    ? p.feedbacks.toLocaleString("ru") + " отзывов"
    : "";
  ...
  card.innerHTML =
    ...
    totalBadge +
    (p.checkedAt
      ? '<span class="change-date">' + formatTime(p.checkedAt) + "</span>"
      : "") +
    "</div>" +
    (p.history && p.history.length > 1
      ? '<div class="sparkline-wrap"><canvas class="sparkline-canvas" height="40"></canvas></div>'
      : "") +
...
function drawSparkline(canvas, prices) {
  ...
  const toX = function (i) {
    return pad + (i / (prices.length - 1)) * (w - pad * 2);
  };
...
function openModal(article) {
  ...
  const statsHtml = [
    p.rating
      ? '<div class="stat-box"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg><span>' +
        p.rating.toFixed(1) +
        "</span></div>"
      : "",
    p.feedbacks
      ? '<div class="stat-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg><span>' +
        p.feedbacks.toLocaleString("ru") +
        "</span></div>"
      : "",
    history.length > 1
      ? '<div class="stat-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 6l-9.5 9.5-5-5L1 18"/><path d="M17 6h6v6"/></svg><span>мин. ' +
        formatPrice(
          Math.min.apply(
            null,
      ...
      : "",
    history.length > 1
      ? '<div class="stat-box"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 18l-9.5-9.5-5 5L1 6"/><path d="M17 18h6v-6"/></svg><span>макс. ' +
        formatPrice(
          Math.max.apply(
            null,
            history.map(function (h) {
      ...
      const diffStr =
        d !== 0
          ? '<span class="history-diff ' +
            ...
            d +
            " ₽</span>"
          : '<span class="history-diff">—</span>';
      return (
        '<div class="history-row">' +
        '<span class="history-cell history-date">' +
        formatDateTime(h.ts) +
        "</span>" +
        '<span class="history-cell history-price">' +
        formatPrice(h.price) +
        " ₽</span>" +
        diffStr +
        "</div>"
  ...
  content.innerHTML =
    ...
    esc(p.name || "Товар " + p.article) +
    "</div>" +
    '<div class="modal-article">арт. ' +
    esc(p.article) +
    "</div>" +
    (p.url
      ? '<a class="modal-open-link" href="' +
        p.url +
    ...
    (diff !== 0
      ? '<div class="price-change ' +
        ...
        (diff > 0 ? "+" : "") +
        diff +
        " ₽ с прошлой проверки</span>" +
        "</div>"
      : '<div class="price-change stable"><span>Цена не изменилась</span></div>') +
    "</div>" +
    ...
    (history.length > 0
      ? '<div class="modal-section">' +
        '<div class="modal-section-title">История изменений <span class="history-count">(' +
        history.length +
        ")</span></div>" +
    ...
    '<a href="' +
    (p.url || "#") +
    '" target="_blank" rel="noopener noreferrer" class="modal-btn btn-secondary btn-full">' +
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' +
    '<path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6v6M10 14L21 3"/>' +
    "</svg>" +
...
function drawFullChart(canvas, history) {
  ...
  const prices = history.map(function (h) {
    return h.price;
  });
  const min = Math.min.apply(null, prices);
  const max = Math.max.apply(null, prices);
  ...
  const toX = function (i) {
    return padX + (i / (prices.length - 1)) * (w - padX * 2);
  };
  const toY = function (v) {
  ...
  // Price labels
  ctx.fillStyle = "rgba(161,161,170,.6)";
...
function syncFromServer() {
  ...
  chrome.runtime.sendMessage({ type: "SYNC_FROM_WORKER" }, function (res) {
    ...
    if (res && res.ok) {
      loadAll();
      showToast("Синхронизировано " + res.count + " товаров", "success");
    } else {
