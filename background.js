...
function getDestFromPage() {
  try {
    const geoStr = localStorage.getItem('wba-geo') || localStorage.getItem('x-wba-geo') || '';
  ...
  if (deliveryMin === null) {
    ...
    if (deliveryData) {
      ...
      if (deliveryMin === null && deliveryData.deliveryTime) {
        const match = String(deliveryData.deliveryTime).match(/(\d+)\s*[-–—]\s*(\d+)/);
        if (match) {
    ...
    if (deliveryMin === null && product.deliveryPeriod) {
      const match = String(product.deliveryPeriod).match(/(\d+)\s*[-–—]\s*(\d+)/);
      if (match) {
        deliveryMin = parseInt(match[1], 10);
        deliveryMax = parseInt(match[2], 10);
      }
...
function extractProductData(article, data) {
  ...
  if (!product) {
    console.warn(`[1000fps] Не удалось найти товар в ответе для ${article}. Структура:`, JSON.stringify(data).slice(0, 500));
    return null;
  }
  ...
  if (!price) {
    console.warn(`[1000fps] Цена не найдена для ${article}. Данные товара:`, JSON.stringify(product).slice(0, 1000));
  }

  // Отладочная информация о количестве и доставке
  console.log(`[1000fps] 📦 ${article}: stock=${stockQuantity}, delivery=${deliveryMin}-${deliveryMax} дн.`);
  ...
  return {
    ...
    description: '',
    brand: product.brand || product.brandName || product.selling?.brand_name || '',
    price,
    originalPrice,
    ...
    _raw: { name: product.name, price: product.price, salePrice: product.salePrice }
  };
}
...
async function getWBTab() {
  try {
    ...
    let tab = wbTabs.find(t => t.status === 'complete' && /https:\/\/(www\.)?wildberries\.ru\//.test(t.url));
...
async function fetchProductInfo(article) {
  ...
  if (!tab) {
    throw new Error('Нет открытых вкладок wildberries.ru. Откройте WB в браузере и попробуйте снова.');
  }
  ...
  for (let i = CACHE.workingEndpoint; i < API_ENDPOINTS.length; i++) {
    ...
    try {
      ...
      try {
        ...
        if (!tabCheck || tabCheck.status !== 'complete' || !tabCheck.url || !tabCheck.url.includes('wildberries.ru')) {
          tab = await getWBTab();
          if (!tab) { lastError = 'Нет доступных вкладок WB'; break; }
        }
      } catch(e) {
      ...
      if (!res) { lastError = 'Нет ответа от вкладки WB'; continue; }
      if (res.error) {
        lastError = res.error;
        if (res.error.startsWith('HTTP ')) {
          console.warn(`[1000fps] ${res.error} для endpoint ${i}, пробуем следующий`);
        }
        continue;
      }
      ...
      if (!product) { lastError = 'Не удалось извлечь данные товара'; continue; }
      if (!product.price) { lastError = 'Цена не найдена в ответе'; continue; }
      ...
      if (
        ...
        if (freshTab && freshTab.id !== tab.id) {
          console.warn(`[1000fps] Вкладка ${tab.id} недоступна, пробуем ${freshTab.id}`);
          tab = freshTab;
          tabSwitchAttempts++;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (result.ok && result.data && result.data.id) {
        ...
        console.log(`[1000fps] ✅ Найдено по wb_article: product_id=${productId}`);
      } else {
    ...
    if (!productId) {
      ...
      const productsRes = await fetch(`${BACKEND}/api/products?search=${article}&limit=100`);
      if (productsRes.ok) {
        ...
        if (productsResult.ok && productsResult.data) {
          ...
          for (const product of productsResult.data) {
            if (product.wb_article === article) {
              ...
              console.log(`[1000fps] ✅ Найдено по search: product_id=${productId}`);
              break;
            }
          }
        }
    ...
    const sourcesRes = await fetch(`${BACKEND}/api/products/${productId}/sources`, {
      headers: { 'Content-Type': 'application/json' }
    });
    
    if (!sourcesRes.ok) {
      console.warn(`[1000fps] ❌ Не удалось загрузить источники: HTTP ${sourcesRes.status}`);
      return [];
    }
    ...
    console.log(`[1000fps] ✅ Загружено ${sources.length} источников для product_id=${productId}:`, 
...
async function fetchProductWithSources(article) {
  ...
  console.log(`[1000fps] Multi-source: ${sources.length} источников для ${article}`);
  
  // Парсим источники по порядку
  for (const source of sources) {
    try {
      console.log(`[1000fps] Пробую источник ${source.source_article} (priority: ${source.priority})`);
      
      const info = await fetchProductInfo(source.source_article);
      
      if (info && info.price) {
        console.log(`[1000fps] ✅ Успех: ${source.source_article} - ${info.price}₽`);
        info.sourceArticle = source.source_article;
        info.sourceId = source.id;
        return info;
      }
      
      console.log(`[1000fps] ❌ Не удалось получить цену из ${source.source_article}`);
      
    } catch (e) {
      console.warn(`[1000fps] Ошибка парсинга ${source.source_article}:`, e.message);
    }
  }
...
async function checkAllPrices() {
  ...
  for (const article of articles) {
    try {
      ...
      if (lastPrice !== null && lastPrice !== info.price) {
        ...
        console.log(`[1000fps] 💰 ${article}: ${lastPrice}₽ → ${info.price}₽ (${diff > 0 ? '+' : ''}${diff}₽)`);
        ...
        try {
          await chrome.notifications.create('wb-' + article + '-' + Date.now(), {
            ...
            message: lastPrice + '₽ → ' + info.price + '₽ (' + (diff > 0 ? '+' : '') + diff + '₽)'
          });
        } catch(e) {}
      }
      ...
      await sendToServer(store.products[article], 'update', 0, info.sourceArticle);

      // Отправляем результаты парсинга на webhook для сохранения как поставщики
      try {
        await sendToWebhook(store.products[article], info.sourceArticle);
      } catch (e) {
        console.warn(`[1000fps] Webhook error for ${article}:`, e.message);
      }
  ...
  // Итоговое сообщение
  const changeMsg = priceChangedCount > 0 ? ` | 💰 Изменилось цен: ${priceChangedCount}` : '';
  const firstMsg = firstPriceCount > 0 ? ` | ✅ Первых цен: ${firstPriceCount}` : '';
  const multiMsg = multiSourceCount > 0 ? ` | 🔀 Multi-source: ${multiSourceCount}` : '';
  console.log(`[1000fps] ✅ Проверка завершена: ${successCount} успешно, ${errorCount} ошибок${changeMsg}${firstMsg}${multiMsg}`);
  ...
  (async function() {
    try {
      if (msg.type === 'PRICE_ADD') {
        ...
        await sendToServer(store.products[msg.article], 'add', 0, info.sourceArticle);

        // Отправляем результаты парсинга на webhook для сохранения как поставщики
        await sendToWebhook(store.products[msg.article], info.sourceArticle);
      ...
      if (msg.type === 'PRICE_REFRESH_ONE') {
        ...
        if (lastPrice !== null && lastPrice !== info.price) {
          const prevPriceNum = typeof lastPrice === 'string' ? parseFloat(lastPrice) : lastPrice;
          const priceNum = typeof info.price === 'string' ? parseFloat(info.price) : info.price;
          console.log(`[1000fps] PRICE_REFRESH_ONE: ${msg.article} ${prevPriceNum}→${priceNum}, diff=${priceNum - prevPriceNum}`);
        ...
        try {
          await sendToServer(store.products[msg.article], 'update', 0, info.sourceArticle);
        } catch (e) {
        ...
        try {
          ...
          console.error('[1000fps] Send to webhook error:', e);
        }
      ...
      if (msg.type === 'SYNC_FROM_WORKER') {
        syncFromServer().then(async () => {
          const store = await getStore();
          sendResponse({ ok: true, count: Object.keys(store.products || {}).length });
        });
        return true; // async
      }
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'wbPriceCheck') {
    ...
    chrome.tabs.query({}, (tabs) => {
      const hasWBTab = tabs.some(t => t.url && t.url.includes('wildberries.ru'));
      if (!hasWBTab) {
        console.log('[1000fps] Нет открытой вкладки WB — пропускаем автопроверку. Откройте wildberries.ru.');
        return;
      }
      checkAllPrices();
    });
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  try {
    ...
    if (productInfo && productInfo.price) {
      ...
      console.log(`[1000fps] 🔍 Сырые данные WB:`, JSON.stringify(productInfo._raw));
      ...
      // Отправляем результаты парсинга на webhook для сохранения как поставщики
      try {
        await sendToWebhook(productInfo, productInfo.sourceArticle);
      } catch (e) {
        console.warn('[1000fps] Webhook error:', e.message);
      }

      console.log(`[1000fps] ✅ Спарсено: ${article} - ${productInfo.price}₽${productInfo.sourceArticle && productInfo.sourceArticle !== article ? ` (источник: ${productInfo.sourceArticle})` : ''}`);
      ...
      if (autoCloseEnabled === true) {
        ...
        console.log(`[1000fps] 🔄 Закрываем вкладку ${tabId} через 2 секунды...`);
        setTimeout(async () => {
          try {
        ...
        console.log(`[1000fps] ℹ️ Автозакрытие выключено, вкладка ${tabId} остаётся открытой`);
      }
...
async function syncFromServer() {
  ...
  try {
    ...
    if (result.ok && result.data && Array.isArray(result.data)) {
      ...
      for (const product of result.data) {
        if (product.article && !store.products[product.article]) {
          ...
          store.products[product.article] = {
            ...
            addedAt: product.addedAt || new Date().toISOString(),
            history: product.price ? [{ price: product.price, ts: new Date().toISOString() }] : []
          };
          ...
          if (product.price && local.price !== product.price) {
            ...
            if (product.history && product.history.length > local.history.length) {
      ...
      await saveStore(store);
      console.log(`[1000fps] Server sync: +${newCount} новых, ~${updatedCount} обновлено`);
...
/**
 * Отправляет результаты парсинга на webhook для сохранения как поставщики
 * @param {Object} productData - Данные товара из парсинга
 * @param {string} sourceArticle - Исходный артикул (если отличается от основного)
 */
async function sendToWebhook(productData, sourceArticle = null) {
  const { backendUrl } = await chrome.storage.local.get('backendUrl');
  const BASE_URL = backendUrl || 'http://localhost:3002';
  const WEBHOOK_URL = `${BASE_URL}/api/admin/parser/webhook`;

  // Находим productId по артикулу
  const article = productData.article || productData.sourceArticle || sourceArticle;
  let productId = null;

  if (article) {
    try {
      const searchRes = await fetch(`${BASE_URL}/api/wb/products?article=${article}`);
      if (searchRes.ok) {
        const searchResult = await searchRes.json();
        if (searchResult.ok && searchResult.data && searchResult.data.id) {
          productId = searchResult.data.id;
          console.log(`[1000fps] 🔍 Найден productId=${productId} для article=${article}`);
        }
      }
    } catch (e) {
      console.warn('[1000fps] Не удалось найти productId по артикулу:', e.message);
    }
  }

  // Формируем результат в формате поставщика
  const source = productData.url || `https://www.wildberries.ru/catalog/${article}/detail.aspx`;
  const price = productData.price ?? null;
  const oldPrice = productData.originalPrice ?? null;
  const inStock = productData.inStock ?? (productData.stockQuantity > 0);
  const stockQuantity = productData.stockQuantity ?? 0;
  const deliveryMin = productData.deliveryMin ?? null;
  const deliveryMax = productData.deliveryMax ?? null;
  const rating = productData.rating ?? null;
  const feedbacks = productData.feedbacks ?? 0;
  ...
  // Генерируем временный jobId для идемпотентности
  const jobId = `webhook_${article}_${Date.now()}`;
  ...
  console.log('[1000fps] 📤 Отправка поставщика на webhook:', article, payload);
  ...
  try {
    ...
    console.log('[1000fps] ✅ Поставщик сохранён через webhook:', article, webhookResult);
    return webhookResult;

  } catch (e) {
    console.warn('[1000fps] ❌ Webhook send error:', e.message);
    // Не блокируем основной поток — ошибка сохраняется в логи
    return null;
  }
}

async function sendToServer(product, action, retryCount = 0, sourceArticle = null) {
  const { backendUrl } = await chrome.storage.local.get('backendUrl');
  ...
  console.log('[1000fps] 📤 Отправка на сервер:', action, product.article, payload);
  ...
  try {
    ...
    const result = await response.json();
    console.log('[1000fps] ✅ Отправлено на сервер:', action, product.article, result);

  } catch (e) {
    clearTimeout(timeoutId);
    const isAbort = e.name === 'AbortError';
    console.log(`[1000fps] ❌ Server send error: ${isAbort ? 'Таймаут (10s)' : e.message}`);
    ...
    if (retryCount < 3) {
      ...
      console.log(`[1000fps] 🔄 Повторная попытка ${retryCount + 1}/3 через ${delay / 1000}с...`);
...
async function flushPendingQueue() {
  ...
  console.log(`[1000fps] 📬 Отправка pending queue: ${pendingQueue.length} записей`);
  ...
  for (const item of pendingQueue) {
    try {
      ...
      if (item.retryCount <= 5) {
        ...
        console.log(`[1000fps] 🗑️ Удалено из queue после 5 попыток: ${item.product?.article}`);
      }
    }
  }

  await chrome.storage.local.set({ pendingQueue: failed });
  console.log(`[1000fps] 📬 Queue: отправлено ${pendingQueue.length - failed.length}, осталось ${failed.length}`);
}